<script lang="ts">
    import { createInputDispatcher } from "../commands";
    import { MAX_GRID_SIZE, MIN_GRID_SIZE } from "$lib/crossword";
    import type { Crossword } from "$lib/types";
    import {
        FlipVertical,
        List,
        Blocks,
        TextCursorInput,
        Save,
        FileUp,
        Undo2,
        Redo2,
    } from "lucide-svelte";
    import Switch from "./ui/switch.svelte";
    import Button from "./ui/button.svelte";
    import Input from "./ui/input.svelte";
    import Toggle from "./ui/toggle.svelte";

    export let crossword: Readonly<Crossword>;
    export let autoSymmetry: boolean;
    export let isGridDesigner: boolean;
    export let undoable: boolean;
    export let redoable: boolean;
    export let isSplit: boolean;
    export let listView: boolean;

    let { size } = crossword;
    $: setSize(crossword.size);

    function setSize(newSize: number) {
        size = newSize;
    }

    const dispatch = createInputDispatcher<{
        undo: null;
        redo: null;
        toggleView: null;
    }>();
</script>

<div
    class="flex h-full w-full flex-wrap-reverse items-start gap-2 border-b border-gray-600 px-2 pb-2 shadow-sm xl:px-6 xl:pb-6 dark:bg-gray-800"
>
    <div class="flex max-w-xl flex-1 flex-nowrap justify-between">
        <fieldset class="flex gap-1 xl:gap-2">
            <Button
                type="button"
                variant="secondary"
                on:click={() => dispatch("undo")}
                disabled={!undoable}
            >
                <Undo2 />
            </Button>

            <Button
                type="button"
                variant="secondary"
                on:click={() => dispatch("redo")}
                disabled={!redoable}
            >
                <Redo2 />
            </Button>
        </fieldset>

        <div class="flex items-center gap-2 xl:gap-6">
            <Switch
                bind:checked={isGridDesigner}
                aria-label={isGridDesigner
                    ? "Switch to text input."
                    : "Switch to grid designer."}
            >
                <Blocks slot="left" />
                <TextCursorInput slot="right" />
            </Switch>
            <Toggle
                bind:pressed={autoSymmetry}
                disabled={!isGridDesigner}
                aria-label={`Toggle auto-symmetry ${autoSymmetry ? "off" : "on"}.`}
            >
                <FlipVertical />
            </Toggle>
            <Input
                id="sizeInput"
                type="number"
                aria-label="Set grid size."
                min={MIN_GRID_SIZE}
                max={MAX_GRID_SIZE}
                class="form-input flex w-24 items-center"
                bind:value={size}
                on:change={() => dispatch("resizeGrid", size)}
            />
        </div>
    </div>

    <div class="flex flex-1 flex-nowrap justify-between">
        {#if isSplit}
            <Toggle
                bind:pressed={listView}
                class="bg-white data-[state='on']:bg-blue-100"
            >
                <List class="h-6 w-6" />
            </Toggle>
        {:else}
            <div aria-hidden="true" class="h-[1px] w-[1px] opacity-0"></div>
        {/if}

        <div class="flex items-center gap-2">
            <Button type="button" variant="secondary">
                <Save />
            </Button>
            <Button type="button">
                <FileUp />
            </Button>
        </div>
    </div>
</div>
